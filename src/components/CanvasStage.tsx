...
export default function CanvasStage() {
  ...
  useEffect(() => {
    ...
    const hitTest = (sx: number, sy: number) => {
      ...
      const { sx: x1, sy: y1 } = screenFromWorld(
        rect.x + rect.width,
        rect.y + rect.height
      );
    ...
    const onPointerMove = (e: PointerEvent) => {
      ...
      if (dragMode === "move") {
        ...
        const ratioizeCorner = (
          ...
          if (anchorAtRight) nx = anchorX - w1;
          else nx = anchorX;
          if (anchorAtBottom) ny = anchorY - h1;
          else ny = anchorY;
          nw = w1;
          nh = h1;
        };
        ...
        switch (activeHandle) {
          case 0: {
            ...
            if (lockAspect)
              ratioizeCorner(right, bottom, candW, candH, true, true);
            else {
          ...
          case 2: {
            ...
            const candH = bottom - wy;
            if (lockAspect)
              ratioizeCorner(left, bottom, candW, candH, false, true);
            else {
              ny = Math.min(wy, bottom);
              nw = Math.max(minW, wx - left);
              nh = bottom - ny;
              nx = left;
            }
            break;
          ...
          case 4: {
            ...
            const candH = wy - top;
            if (lockAspect)
              ratioizeCorner(left, top, candW, candH, false, false);
            else {
              nw = Math.max(minW, wx - left);
              nh = Math.max(minH, wy - top);
              nx = left;
              ny = top;
            }
            break;
          ...
          case 6: {
            ...
            const candH = wy - top;
            if (lockAspect)
              ratioizeCorner(right, top, candW, candH, true, false);
            else {
              nx = Math.min(wx, right);
              nw = right - nx;
              nh = Math.max(minH, wy - top);
    ...
    const onWheel = (e: WheelEvent) => {
      ...
      useCameraStore
        .getState()
        .setCamera({ scale: newScale, tx: newTx, ty: newTy });
    };
  ...
  return (
    <div className="w-full h-full canvas-container">
      <canvas
        ref={canvasRef}
        className="w-full h-full block bg-[var(--background)]"
      />
    </div>
  );
}
